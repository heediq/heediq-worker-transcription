...
- **The CUDA base image is digest-pinned in both Dockerfiles.** `nvidia/cuda` repushes its mutable tags, and any repush silently busts the whole build cache (pip-install + model-bake layers) — a full uncached rebuild once ran 5.5h and hit the CI job's 6h wall. Both Dockerfiles pin `…@sha256:…`; keep them in lockstep and bump the digest deliberately when changing CUDA versions (resolve one with `docker buildx imagetools inspect <tag> --format '{{.Manifest.Digest}}'`). The build job also carries `timeout-minutes: 45` and builds the two tiers as a parallel `fail-fast: false` matrix, so a stall in one tier fails fast with logs instead of blocking the other.
- **SSM parameters must be seeded before the first infra deploy.** TranscriptionStack resolves image tags from `/heediq/transcription/{free,paid}-image-tag` via a CloudFormation dynamic reference. These parameters must exist in each workload account before the first `cdk deploy TranscriptionStack`. `heediq-infra/scripts/setup.sh` section 3 seeds them automatically (idempotent — skips if already set). If seeding manually: `aws ssm put-parameter --name /heediq/transcription/free-image-tag --value free --type String`. After CI's first promote run, CI owns the value and `setup.sh` will no longer overwrite it. See `heediq-infra/README.md`'s "Initial Setup" / "Setting up a new environment from scratch" for the canonical first-deploy sequence this fits into.
